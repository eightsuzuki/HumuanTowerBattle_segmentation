...
from utils import denormalize, hwc_to_chw, normalize, resize_imgs
...
def predict_img(net, gpu, height, width):
    ...
    while True:
        ...
        # ウィンドウに画像を表示 --- (*3)
        img = cv2.rectangle(img,(155,-5),(485,485),(255,0,0),5)
        ...
        if cv2.waitKey(1) == 32:
            ...
            with torch.no_grad():
                ...
                result = cv2.resize(result.astype(np.float32) , (org_img.size[0], org_img.size[1]))
                cv2.imwrite('../../tower_battle/Assets/result.png', result)
...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load(args.model), loc: 'cpu')
    print("Model loaded!\n")
